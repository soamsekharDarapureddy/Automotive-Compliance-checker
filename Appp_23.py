...
# 3. MASSIVELY EXPANDED Component Knowledge Base
...
st.set_page_config(page_title="E-Bike Test & Requirement Tool", layout="wide")
st.title("Automotive E-Bike Test Verification & Requirement Generator")
...
if option == "Upload & Verify Test Report":
    ...
    if st.button("Find Component Info"):
        ...
        found_info = None
        found_key = None
        if part_number_to_find: # Only search if input is not empty
            for key in COMPONENT_KNOWLEDGE_BASE:
                if key in part_number_to_find:
                    found_key = key
                    found_info = COMPONENT_KNOWLEDGE_BASE[key]
        ...
        if found_info:
            ...
            st.session_state.found_component = {} # Clear previous findings
            st.warning("Part number not found in knowledge base. You can add it manually below.")
    
    # --- Part 2: The Form for Adding to Project ---
    st.markdown("---")
